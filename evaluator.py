...
class Evaluator():
    ...
    def evaluate_alignment_loss(self, model: TutorialLLM, reference_model: TutorialLLM, beta: float = 0.1):
        ...
        positive_loss_sum = 0
        negative_loss_sum = 0
        reference_positive_loss_sum = 0
        reference_negative_loss_sum = 0
        positive_weight = 0.8
        negative_weight = 0.2
        # Get a batch generator of finetune data
        ...
        for k, batch in enumerate(batch_generator):
            ...
            positive_loss_sum += positive_loss.mean().item()
            negative_loss_sum += negative_loss.mean().item()
            reference_positive_loss_sum += reference_positive_loss.mean().item()
            reference_negative_loss_sum += reference_negative_loss.mean().item()
            ...
            loss_sum += loss.item()
            reward_margin_sum += reward_margin.mean().item()

        positive_loss_mean = positive_loss_sum / (k + 1)
        negative_loss_mean = negative_loss_sum / (k + 1)
        reference_positive_loss_mean = reference_positive_loss_sum / (k + 1)
        reference_negative_loss_mean = reference_negative_loss_sum / (k + 1)

        positive_reward_mean = reference_positive_loss_mean - positive_loss_mean
        negative_reward_mean = negative_loss_mean - reference_negative_loss_mean
        reward_margin_mean = positive_weight * positive_reward_mean + negative_weight * negative_reward_mean
        reward_margin_mean = torch.Tensor([reward_margin_mean])
        loss_mean = - F.logsigmoid(beta * reward_margin_mean).item()
        print(f'Positive loss: {positive_loss_mean:.4f}, Negative loss: {negative_loss_mean:.4f}')
        print(f'Reference positive loss: {reference_positive_loss_mean:.4f}, Reference negative loss: {reference_negative_loss_mean:.4f}')
        print(f'Positive reward: {positive_reward_mean:.4f}, Negative reward: {negative_reward_mean:.4f}')
        print(f'loss mean: {loss_mean:.4f}, reward margin mean: {reward_margin_mean.item():.4f}')
        loss = loss_sum / (k + 1)
